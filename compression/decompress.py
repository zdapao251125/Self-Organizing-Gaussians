...
def decompress_single_to_ply(compressed_model_path, template_gaussians=None):

    metrics_dict = {"Size [Bytes]": get_size_of_files_in_dir(compressed_model_path)}

    decompressed_gaussians = run_single_decompression(
        compressed_model_path, template_gaussians=template_gaussians
    )
...
def decompress_all_to_ply(compressions_dir, template_gaussians=None):

    for compressed_dir in os.listdir(compressions_dir):
        if not os.path.isdir(os.path.join(compressions_dir, compressed_dir)):
            continue
        decompress_single_to_ply(
            os.path.join(compressions_dir, compressed_dir),
